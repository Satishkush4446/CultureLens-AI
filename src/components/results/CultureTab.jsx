...
export default function CultureTab({ tripData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Do's & Don'ts */}
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-3">Traveler Etiquette</h3>
            ...
            <GlassCard key={idx} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg bg-emerald-500/5 border border-emerald-500/10 p-3">
                  ...
                  <p className="text-xs text-slate-300">{item.do}</p>
                </div>
                <div className="rounded-lg bg-red-500/5 border border-red-500/10 p-3">
                  <p className="text-xxs font-bold text-red-400 uppercase tracking-widest mb-1">✕ Don't</p>
                  <p className="text-xs text-slate-300">{item.dont}</p>
                </div>
              </div>
              {item.context && (
                <p className="text-xxs text-slate-500 leading-relaxed border-t border-white/5 pt-2">
                  <span className="text-slate-400 font-medium">Why: </span>{item.context}
                </p>
              )}
            </GlassCard>
          ))}
